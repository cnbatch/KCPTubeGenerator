...
void KCPTubeGeneratorUserInput::PutTextOnScreen()
{
	...
	m_checkBox_client_mtu->SetLabelText(setting_options_id_to_text[languages::setting_options_id::kcp_mtu]);
	m_checkBox_relay_mtu_fc->SetLabelText(setting_options_id_to_text[languages::setting_options_id::kcp_mtu]);
	m_checkBox_relay_mtu_ts->SetLabelText(setting_options_id_to_text[languages::setting_options_id::kcp_mtu]);
	m_checkBox_server_mtu->SetLabelText(setting_options_id_to_text[languages::setting_options_id::kcp_mtu]);
...
void KCPTubeGeneratorUserInput::CreateClientSettingFile()
{
	...
	if (m_checkBox_client_dport_refresh->IsChecked())
		client_settings += ("dport_refresh=" + std::to_string(m_spinCtrl_client_dport_refresh->GetValue()) + "\n");

	if (m_checkBox_client_udp_timeout->IsChecked())
		client_settings += ("udp_timeout=" + std::to_string(m_spinCtrl_client_udp_timeout->GetValue()) + "\n");

	if (m_checkBox_client_keep_alive->IsChecked())
		client_settings += ("keep_alive=" + std::to_string(m_spinCtrl_client_keep_alive->GetValue()) + "\n");

	if (m_checkBox_client_mux_tunnels->IsChecked())
		client_settings += ("mux_tunnels=" + std::to_string(m_spinCtrl_client_mux_tunnels->GetValue()) + "\n");
	...
	if (m_checkBox_client_mtu->IsChecked())
		client_settings += ("kcp_mtu=" + std::to_string(m_spinCtrl_client_mtu->GetValue()) + "\n");
...
void KCPTubeGeneratorUserInput::CreateRelaySettingFile()
{
	...
	if (m_checkBox_relay_udp_timeout_fc->IsChecked())
		relay_listener += ("udp_timeout=" + std::to_string(m_spinCtrl_relay_udp_timeout_fc->GetValue()) + "\n");

	if (m_checkBox_relay_keep_alive_fc->IsChecked())
		relay_listener += ("keep_alive=" + std::to_string(m_spinCtrl_relay_keep_alive_fc->GetValue()) + "\n");

	if (m_checkBox_relay_mtu_fc->IsChecked())
		relay_listener += ("kcp_mtu=" + std::to_string(m_spinCtrl_relay_mtu_fc->GetValue()) + "\n");
	...
	if (m_checkBox_relay_dport_refresh->IsChecked())
		relay_forwarder += ("dport_refresh=" + std::to_string(m_spinCtrl_relay_dport_refresh->GetValue()) + "\n");

	if (m_checkBox_relay_udp_timeout_ts->IsChecked())
		relay_forwarder += ("udp_timeout=" + std::to_string(m_spinCtrl_relay_udp_timeout_ts->GetValue()) + "\n");

	if (m_checkBox_relay_keep_alive_ts->IsChecked())
		relay_forwarder += ("keep_alive=" + std::to_string(m_spinCtrl_relay_keep_alive_ts->GetValue()) + "\n");

	if (m_checkBox_relay_mtu_ts->IsChecked())
		relay_forwarder += ("kcp_mtu=" + std::to_string(m_spinCtrl_relay_mtu_ts->GetValue()) + "\n");
...
void KCPTubeGeneratorUserInput::CreateServerSettingFile()
{
	...
	if (m_checkBox_server_mtu->IsChecked())
		server_settings += ("udp_timeout=" + std::to_string(m_spinCtrl_server_mtu->GetValue()) + "\n");
